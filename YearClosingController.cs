...
namespace Jobs.Areas.Rug.Controllers
{
    ...
    public class YearClosingController : ReportController
    {
        ...
        private Boolean FPost(DateTime CloseDate)
        {
            ...
            int WVORD = 458, WFORD = 455, WVORH = 637;
            int WVREC = 448;
            int WVCNL = 449;
            ...
            try
            {
                ...
                mQry = "SELECT JOH.JobOrderHeaderId, Max(convert(INT,JW.IsSisterConcern)) AS IsSisterConcern,  Max(C.CostcenterName) AS CostcenterName, Max(CE.ProductId) AS ProductId, Max(LA.LedgerAccountId) AS LedgerAccountId, Max(JOH.Remark) AS Remark, Max(JOH.UnitconversionForId) AS UnitconversionForId, Max(JOH.ActualDueDate) AS ActualDueDate, Max(JOH.DocTypeId) AS DocTypeId, Max(JOH.DocDate) AS DocDate, Max(JOH.DocNo) AS DocNo, Max(JOH.DivisionId) AS DivisionId, Max(JOH.SiteId) AS SiteId, Max(JOH.DueDate) AS DueDate, " +
                    ...
                    " FROM " +
                    " ( " +
                    "       SELECT H.JobOrderHeaderId FROM Web.JobOrderHeaders H WITH (Nolock) WHERE H.DocTypeId IN ( " + WVORD + "," + WFORD+","+ WVORH + " )  " +
                ...
                if (DtJobOrder.Rows.Count > 0)
                {
                    ...
                    for (I = 0; I < DtJobOrder.Rows.Count; I++)
                    {
                        ...
                        if (IsCostcenterExist == 0)
                        {
                            ...
                            //New Added Site Division Filter date On 20/Mar/19
                            mQry = " INSERT INTO Web.CostCenterStatus (CostCenterId) " +
                                    " SELECT CostCenterId FROM Web.CostCenters WHERE CostCenterName = '" + NewCostcenterNo + "' And SiteId='"+ SiteId + "' and DivisionId='"+ DivisionId + "' ";
                            Cmd = new SqlCommand(mQry, Con);
                            if (Cmd.Connection.State != ConnectionState.Open)
                                Cmd.Connection.Open();
                            Cmd.Transaction = ETrans;
                        ...
                        // *****************  For Getting Pending Job OrderLine In this Added L.LossQty to isnull(L.LossQty,0) as LossQty date On 20/Mar/19
                        mQry = " SELECT H.JobOrderHeaderId, H.DocTypeId, H.DocNo, H.DocDate, H.JobWorkerId, H.ProcessId, " +
                                ...
                                " L.NonCountedQty, isnull(L.LossQty,0) as LossQty, L.Specification, L.UnitConversionMultiplier, L.StockId AS StockId, L.ProductUidHeaderId AS ProductUidHeaderId, L.Sr, " +
                        ...
                        if (DtJobOrderLine.Rows.Count > 0)
                        {
                            ...
                            for (J = 0; J < DtJobOrderLine.Rows.Count; J++)
                            {
                                ...
                                Cmd.Transaction = ETrans;
                                Cmd.ExecuteNonQuery();
                                // For Insert For ProductUidHeaderId respect with Orderlines 
                                ...
                                if (SiteId == 17)
                                {
                                    FTransferProductUidsAndJoborderLine((int)DtJobOrderLine.Rows[J]["JobOrderLineId"], ETrans);
                                }
        ...
        private string FGetNewCostcenterNo(DataTable DtJobOrder, int Row, SqlTransaction ETrans)
        {
            ...
            if (RunningPurja == "")
            {
                ...
                mQry = "SELECT PC.ProductCategoryName FROM web.Products FP WITH (Nolock) " +
                ...
                else if (DtJobOrder.Rows[Row]["DivisionId"].ToString() == "1" && DtJobOrder.Rows[Row]["SiteId"].ToString() == "17")
                {
                    ...
                    else
                    {
                        if (bConstruction.ToUpper().Contains("KELIM"))
                        {
                            bStartFrom = 1;
                            //strCondConstruction = " AND PC.ProductCategoryName =  '" + bConstruction + "' AND IsNull(JW.IsSisterConcern,0) = 0 ";
                            strCondConstruction = " AND PC.ProductCategoryName In ('KELIM','KELIM ACRYLIC','KELIM POUF') AND IsNull(JW.IsSisterConcern,0) = 0 ";
                        }
                        else
                        {
                            //strCondConstruction = " AND PC.ProductCategoryName Not In ('TUFTED','HANDLOOMS')  AND IsNull(JW.IsSisterConcern,0) = 0 ";
                            strCondConstruction = " AND PC.ProductCategoryName In ('SHAG','JUTE KNOTTED','Jute Plain')  AND IsNull(JW.IsSisterConcern,0) = 0 ";
                            bStartFrom = 5001;
                        }
                ...
                mQry = "SELECT IsNull(Max(Convert(BIGINT,replace(C.CostCenterName,'" + Prefix + "',''))),0) " +
                         ...
                         //"      AND H.DocDate  = '24/Dec/2017' " +
                        // "      AND H.DocDate  = '31/Mar/19' " +
                         " ) H " +
                         ...
                         " LEFT JOIN web.JobOrderLines L WITH (Nolock) ON L.JobOrderHeaderId = H.JobOrderHeaderId  " +
                         " LEFT JOIN web.Products  FP WITH (Nolock) ON FP.ProductId = L.ProductId  " +
                         " LEFT JOIN web.ProductCategories PC WITH (Nolock) ON PC.ProductCategoryId = FP.ProductCategoryId " +
        ...
        public void FTransferProductUidsAndJoborderLine(int OldJobOrderLineId, SqlTransaction ETrans)
        {
            #region wotking On Barcode 

            //for Insert ProductuidsHeaderId
            mQry = " Insert into Web.ProductUidHeaders " +
                    " SELECT max(JOL.ProductId) as ProductId ,NULL AS Dimension1Id,NULL AS Dimension2Id,NULL AS LotNo,max(JOHN.JobOrderHeaderId) as GenDocId, " +
                    ...
                    " FROM  Web.JobOrderLines Jol WITH(Nolock) " +
                    " LEFT JOIN web.JobOrderJobOrders JO WITH(Nolock)  ON JO.GenJobOrderHeaderId = Jol.JobOrderHeaderId" +
                    ...
                    " WHERE 1=1  AND(JRL.JobReceiveLineId IS NULL OR JRL.JobReturnLineId IS NOT NULL) AND  JOCL.JobOrderCancelLineId IS NULL" +
                    " AND Jol.JobOrderLineId = " + OldJobOrderLineId + " " +
        ...
        public void InsertProductUids(int OldJobOrderLineId, SqlTransaction ETrans)
        {
            mQry = "UPDATE Web.ProductUids " +
                    " SET Web.ProductUids.LastTransactionDocId = V1.LastTransactionDocId, " +
                    " Web.ProductUids.LastTransactionDocNo = V1.LastTransactionDocNo, " +
